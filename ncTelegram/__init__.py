...
class Telegram_ui:
    def __init__(self, conf):
        ...
        palette = [('status_bar', self.conf['style']['status_bar_fg'], self.conf['style']['status_bar_bg']), # noqa
        ...
        # message buffer init
        # msg_buffer is where incomming message go before they have been printed # noqa
        self.msg_buffer = {}
        # msg_archive is where messages go once they are processed and have the proper layout (urwid list) # noqa
        self.msg_archive = {}
        ...
        # Right pannel
        self.right_side = urwid.Pile([self.msg_widget, (2, self.msg_send_widget)]) # noqa

        vert_separator = urwid.AttrMap(urwid.Filler(urwid.Columns([])), 'status_bar') # noqa
        ...
        self.main_loop = urwid.MainLoop((self.main_columns), palette, unhandled_input=self.unhandle_key, screen=urwid.raw_display.Screen()) # noqa
    ...
    def display_notif(self, msg):
        if self.conf['general']['notification']:
            ...
            if msg['receiver']['type'] == 'user':
                ...
                sender = msg['receiver']['name'] + ": " + msg['sender']['first_name'] # noqa
            Notify.Notification.new('', '<b>' + sender + '</b>\n' + text, self.image).show() # noqa
    ...
    def print_title(self):
        ...
        if total_msg_waiting == 0:
            ...
            sys.stdout.write("\x1b]2;ncTelegram (" + str(total_msg_waiting) + ")\x07") # noqa
    ...
    def open_file(self, path):
        if self.conf['general']['open_file'] and path is not None:
            subprocess.Popen(['xdg-open', path], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL) # noqa
    ...
    def stop_Telegram(self):
        ...
        self.sender.terminate()  # not let the cli end close first -> avoid bind: port already in use. # noqa
        self.receiver.stop()

        self.sender.safe_quit()
        if self.tg._check_stopped():
            return None
        ...
        self.sender.stop()  # quit and safe quit are done, we don't need the sender any longer. # noqa
    ...
    def unhandle_key(self, key):
        if key == self.conf['keymap']['quit']:
            ...
            if 1 == self.main_columns.contents[0][1][1]:    # check if already hidden # noqa
                # hidding
                self.main_columns.contents[0] = (self.main_columns.contents[0][0], ('given', 0, False)) # noqa
                self.main_columns.contents[1] = (self.main_columns.contents[1][0], ('given', 0, False)) # noqa
            else:
                self.main_columns.contents[0] = (self.main_columns.contents[0][0], ('weight', 1, True)) # noqa
                self.main_columns.contents[1] = (self.main_columns.contents[1][0], ('given', 1, False)) # noqa
            ...
            path = self.download_media(self.last_media[self.current_chan['id']]) # noqa
            self.open_file(path)
