...
class ChanWidget(urwid.ListBox):
    ...
    def get_new_chan_list(self):
        ...
        for i in range(len(self.chans)):
            ...
            if chan['peer_type'] == 'user':
                if 'when' in chan:
                    self.Telegram_ui.online_status[chan['id']] = (chan['when'], False) # noqa
                else:
    ...
    def update_chan_list(self):
        ...
        self.chan_list = urwid.SimpleFocusListWalker([urwid.AttrMap(urwid.Text("Chan list:"), 'status_bar')]) # noqa
        ...
        pos += 1
        self.chan_list.insert(pos, urwid.AttrMap(urwid.Divider('─'), 'separator')) # noqa
        # pos += 1
        ...
        if list_buff != list_chan:
            pos += 1
            button = NewButton('⬇  Download message buffer', self.Telegram_ui.fill_msg_buffer) # noqa
            self.chan_list.insert(pos, button)
    ...
    def chan_change(self, button, chan):
        ...
        self.Telegram_ui.msg_send_widget.buffer_writing_text[prev_cmd] = prev_msg # noqa
